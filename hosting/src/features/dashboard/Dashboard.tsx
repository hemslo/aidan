    ...
    return (
        <Stack spacing={3} sx={{justifyContent: 'center'}}>
            {labelsStatus === 'loading'
                ? <CircularProgress/>
                : <Labels canEdit={signInCheckResult?.hasRequiredClaims} labels={labels}
                          labelsCollection={labelsCollection}/>}
            <Snapshots canEdit={signInCheckResult?.hasRequiredClaims} snapshotsCollection={snapshotsCollection}
                       labels={labels}/>
        </Stack>
    );
};
